...
# def add_question(type, data, seed):
#     template = QuestionTemplate.objects.get(pk=type.question_template)
#     q = Question(title = seed, maximum_grade = type.maximum_grade, question = data['question'], question_template = template)
#     q.save()
#     question = Question.objects.get(title=seed)
#     a = PotentialAnswer(question = question, answer = data.answer, is_correct = True)
#     a.save()
#     return question

# def get_question_generation_type(id):
#     return GeneratedQuestionType.object.get(pk=id)
...
def generate_question(seed_value):
    ...
    def cardinalityToText(card):
        if card == "0..1" or card == "0@1":
            ...
            return random.choice([" may be related to many ", " has multiple relationships with ", " has zero or more connections with " ])
        elif card == "1..*":
            return random.choice([" must be related to at least one but possibly many ", "is associated with one or more "])
    ...
    def relationshipToText(r):
        ...
        # Special case for recursive relationship        
            st = "[" + e1 + "](" + e1 + ")" + random.choice([" has a recursive relationship with itself. ", " has a self-relationship. "])
...
___results___ = generate_question(___seed_value___)
'''

def generate_seed():
    return random.randrange(sys.maxsize)

def generate_question(seed_value, question_type):
    # generated_question_type = get_question_generation_type(question_type)
    
    result = compile(code, 'generate', 'exec')
    args = {
    ...
    # return add_question(generated_question_type, question_data, seed_value)

print(generate_question(generate_seed(), '' ))